...
language_casks=("java" "caskroom/versions/java7 groovy")
container_formulas=()
container_casks=("virtualbox" "docker" "kitematic" "docker-completion" "docker-compose-completion") # "docker-machine-completion")
...
term_casks=("coolterm" "tunnelblick")
...
for cask in "${web_casks[@]}" "${language_casks[@]}" "${container_casks[@]}" "${ide_casks[@]}" "${devops_casks[@]}" "${repository_casks[@]}" "${communications_casks[@]}" "${term_casks[@]}"; do
